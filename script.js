...
class Person {
    ...
    // Desc : updates stats that have to do with being masked
    addMask() {
      this.mask = true;
    ...
    // Desc : updates stats that have to do with being vaccined
    addVaccine() {
      this.vaccine = true;
    ...
    build() {
      ...
      for (let x = 0; x < this.gridHeight; x++) for (let y = 0; y < this.gridWidth; y++) {
        var div = document.createElement("canvas");
            // created canvas element representing a cell of the grid
        var tempStr = `${x} + ${y}`; // id for the canvas element
        div.id = tempStr;
        div.height = 30;
        div.width = 30;
...
// Desc : returns a list of randomly chosen people to be masked
...
// Desc : returns a list of randomly chosen people to be vaccinated
...
/* Simulation Output Functions (Integrates User Input) */
...
/* Main Code / Initial Setup for Simulation */
